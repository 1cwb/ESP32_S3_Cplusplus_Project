...
// 重启系统  
void MPU6050Dmp::system_reset(void)
{
    esp_restart();
}


void MPU6050Dmp::run_self_test(void)
{
    int result;
 ...
 * function.
 */
void MPU6050Dmp::gyro_data_ready_cb(void)
{
    hal.new_gyro = 1;
}
...
/*uint8_t mpu_init_i2c(void)
{
    ...
    config_.sda_io_num = MPU_I2C_SDA;
    config_.scl_io_num = MPU_I2C_SCL;
    config_.sda_pullup_en = GPIO_PULLUP_ENABLE;
    config_.scl_pullup_en = GPIO_PULLUP_ENABLE;
    config_.master.clk_speed = 200000;
    
    esp_err = i2c_param_config(I2C_NUM_0, &config_);
    printf("i2c_param_config: %d \n", esp_err);

    esp_err = i2c_driver_install(I2C_NUM_0, I2C_MODE_MASTER, 0, 0, 0);
    printf("i2c_driver_install: %d \n", esp_err);
}*/


// 初始化mpu6050
uint8_t MPU6050Dmp::mpu_dmp_init(void)
{
    //if (mpu_init_i2c() != 0)
    //    return 1;
    int result;
...
// 获取数据，给外部调用 
uint8_t MPU6050Dmp::dmp_get_data(float* pitch, float* roll, float* yaw)
{
